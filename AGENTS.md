...
Chunk Processing
    ...
    ├─► YES: RawEntityReplay / convertRawEventsToEntities (shared raw
    │   lifecycle replay; validated segments feed one stateful
    │   EntityConverter) → Bulk Database Insert (bulkPut) into
...
- **EntityConverter state**: the legacy one-shot `convertEventsToEntities()` call tracks hand boundaries inside that invocation and therefore must NOT be called independently per chunk. Chunked rebuilds use one `RawEntityReplay` instance for the whole replay: `convertChunk()` preserves raw lifecycle and converter state across chunks, delegates validated segments to `EntityConverter.convertEventChunk()`, and calls `flush()` exactly once at the end. Never create a new converter/replay per page.
- **EntityConverter/HandLogProcessor never see raw, unvalidated rows**: both read required fields (e.g. `EVT_DEAL.Game.SmallBlind`) via unguarded `switch (event.ApiTypeId)` dispatch, with no `default:` case protecting against a well-known-ApiTypeId-but-malformed payload. Every entity-producing call site that reads from `apiEvents` (cloud rebuild, manual rebuild, empty/non-empty import) preserves complete raw equal-ms groups through replay ordering and feeds them to the shared `RawEntityReplay` path; it applies raw 201/203/308/309 lifecycle boundaries and only passes validated application segments to `EntityConverter.convertEventChunk()`. Non-entity unpaged consumers may still use `orderAndFilterApplicationEventsForReplay()`. Re-validation on every rebuild is also the *entire* recovery mechanism for a PokerChase schema break — a later schema fix makes previously-unparseable rows parse on the next rebuild automatically, no promotion step required.
...
- **Update Timing**:
  ...
  - Automatic battle-type changes: advance `autoBattleTypeFilterRevision` and send a revision-only port control immediately, without rebroadcasting the previous session's lineup before the next `EVT_DEAL`. `App.tsx` uses that revision to refresh open positional/recent-hands panels once; the later stats payload carries the same revision and is deduplicated. An accepted local `EVT_ENTRY_CANCELLED`(203) in automatic mode serializes an empty stats result so the background cache and HUD cannot retain a cancelled/previous session indefinitely.
...
- **Validation gates the pipeline, never storage** (Raw Event Lake — see Design Principles #16 and `docs/architecture.md`): the content-deduplicating raw merge in `src/background/event-ingestion.ts` runs before `parseApiEvent`/`isApplicationApiEvent` and stores anything with a numeric `timestamp`+`ApiTypeId` — non-application events (202/205 keepalive/timer), ApiTypeIds unknown to `apiEventSchemas`, and app-type events that currently fail to parse are all persisted. The same event is only forwarded to `eventLogger`/`handLogStream`/`handAggregateStream`/`realTimeStatsStream` when it *does* parse as a known application event. Any code path that reads raw `apiEvents` rows and feeds entities into `EntityConverter` must preserve raw group size through replay ordering and use the shared `RawEntityReplay` path, which applies raw lifecycle boundaries before re-validating segments. Hand-log-only unpaged reads use `orderAndFilterApplicationEventsForReplay()`; never pre-filter entity-rebuild chunks with `filterValidApplicationEvents()` before `RawEntityReplay` sees them.
