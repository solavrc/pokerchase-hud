...
19. **Active-port token (game-enforced single session)**: PokerChase enforces exclusive login: one account session has exactly one game tab in use at a time. Across 415k captured hands there was no simultaneous two-table delivery; all 568 observed hand preemptions were sequential, with a minimum 12-second gap. The port that most recently delivered a WebSocket-origin game event MUST be the sole ACTIVE port. Other connected ports are relics: live stats/realtime updates MUST NOT be delivered to them, their state MUST NOT be consulted, and their last rendered display MUST NOT be cleared. Before any token generation exists after a Service Worker start, background-origin aggregate refreshes MUST fall back to all connected game ports so filter, rebuild, import, and auto-sync recovery results are not silently dropped; a reconnect-pending generation is not this no-token case. An event from another tab or document generation MUST transfer the token immediately; a relic MAY reclaim it the same way, and that generation change MUST reset the single current-hand `RealTimeStatsStream` before the event enters the pipeline. A `RuntimePortManager` reconnect from the same `tabId` + `documentId`, connected within the bounded reconnect window, is a replacement transport for the same generation and MUST preserve its current-hand stream, DEAL context, activity, and account. Aggregate stats and their seat-rotation DEAL MUST come from the same ACTIVE generation. A stored raw 309 MUST clear the current-hand stream/cache independent of Zod success while retaining aggregate stats. Replay fetch is allowed only when the current ACTIVE generation is explicitly out of session. A missing token after Service Worker startup, a reconnect-pending generation, and an unknown ACTIVE state MUST all be treated as in-session until a deduplicated game event establishes activity; each queued HandId retains the ACTIVE account observed when it was queued. The always-injected WebSocket hook MUST retain page-world activity from successful 201, seated 303, 308, successful `Code=0` 203, and 309 boundaries. An ApiTypeId 203 response MUST end activity only when `Code === 0`; a nonzero or missing Code MUST remain in-session. The replay bridge MUST NOT start HTTP unless that state is explicitly inactive and MUST independently abort every in-flight request on a start boundary. A stored, non-duplicate start MAY send a generic cancel to all game ports as an auxiliary cross-tab and SW-restart guard; a raw-write-failed start MUST do the same fail-closed, and a content-script port disconnect MUST invalidate that worker's unsent, queued, and in-flight replay requests. These paths MUST NOT use request-specific controller maps or decide whether a result may be stored. Replay responses MUST pass through the serialized event-ingestion queue, and current activity MUST be rechecked after the synthetic-write transaction's first read clears any write-lock wait and immediately before the first 90001 write begins. Replay HTTP MUST remain bound to a per-Service-Worker epoch as an auxiliary ownership guard for SW restarts, not as the primary fairness gate. A different tab delivering within 10 seconds of the prior ACTIVE event MUST emit a diagnostic warning only; an F5 reload candidate with the same `tabId` MUST NOT trigger it, and this sentinel MUST NOT add simultaneous-session behavior.
    **For reviewers**: multi-session is not a game feature — PokerChase never runs two sessions for one account, so review findings premised on two tabs playing simultaneously are invalid by design; the sentinel warning above is the only handling such a scenario gets. The legitimate multi-tab scenarios are all sequential: tab generation handover, same-tab reconnect (state migration, see above), and play/spectate switching.
...
- Three independent stream paths receive the same events simultaneously; one current-hand `RealTimeStatsStream` follows the ACTIVE-port token, resets on a different tab/document generation, and survives the same content script's bounded transport reconnect
...
- **HUD header anchoring** (sola要望 2026-08「HUDのヘッダーを定位置に固定できないかな？」): the header row (player-type icon / player name / SPR / the `▸` positional and `≡` recent-hands triggers) keeps a FIXED screen position for a seat regardless of how the body below it expands. `Hud.tsx`'s outer container is an **anchor box** of constant size (`HUD_WIDTH` × `HUD_ANCHOR_HEIGHT` = 240×52px, pre-`scale`) and the panel hangs inside it at `position: absolute; top: 0`, so every expansion grows **downward only**. Without the constant box the container's `translate(-50%, -50%)` centers the panel on the seat anchor, and any height change ΔH moves the header up by ΔH/2 — measured in Chrome 151 at 1726×996: 9.79px for 簡易→詳細, 47.62px for the positional drill-down, 63.96px for 直近ハンド, i.e. up to 73.75px of total drift. That drift is what makes the `≡` hamburger unhittable when panels from adjacent seats overlap. Rules for changing this area:
  - The panel MUST stay out of the container's normal flow. Reverting it to `position: relative` restores content-dependent centering and silently reintroduces the bug (pinned by `Hud.test.tsx`'s 「ヘッダーの定位置固定」 block, which asserts the anchor-box geometry is byte-identical across compact / inline-expanded / full-grid / drill-down-open / empty-seat renders).
  - `HUD_ANCHOR_HEIGHT` is the **measured** default-configuration panel height (`hudDisplayMode: 'compact'`, drill-downs closed), chosen so no existing user's HUD moves. It also reinterprets nothing: the persisted `hudPosition_<seat>` still means "center of the anchor box". Re-measure in a real browser before changing it — do not estimate (same lesson as the recent-hands column widths above).
  - Tradeoff, accepted rather than clamped: because growth is downward-only, a bottom-seat panel reaches further down than it used to (previously half the growth went upward). At the game's 1726×996 logical viewport and `scale: 1` the worst measured case (詳細グリッド + 直近ハンド open on a `top: 70%` seat) ends at y≈871 of 996, so nothing clips; at large `uiScale` or a short window it can reach the bottom edge, exactly as the pre-existing (halved) overflow could. The HUD has no size-aware viewport clamp — `useDraggable` clamps only the drag position to 0–90% — and none was added: clamping the panel would have to move either the header (the thing being fixed) or the body out from under it.
...
- **Busted-player dim** (sola spec, 2026-07; retention expanded 2026-08): `App.tsx` keeps a per-*display-seat* cache (`dimCacheRef`, keyed by the same post-rotation index `Hud` uses, `seat-${actualSeatIndex}`) of each seat's last live `ExistPlayerStats`. When a fresh trusted lineup shows `-1` at a seat that has a cached entry, `Hud` renders that cached snapshot muted (`opacity: 0.45` on the whole panel + an orange "離席" badge in the header, `HudHeader.tsx`/`Hud.tsx`'s `isDimmed` prop) instead of collapsing to "Waiting for Hand...". Full opacity is restored on hover so drill-down interactions stay comfortable; drill-down panels (positional/recent-hands) keep working off the cached `playerId` regardless. Session end MUST NOT clear these aggregate HUD snapshots or their open drill-downs; the local `PokerChaseSessionEndEvent` clears only current-hand real-time values such as SPR and pot odds. A successful raw `EVT_ENTRY_QUEUED` (201), or the first trusted hero-seated DEAL after session end when that 201 notification was missed, MUST establish the next session boundary and discard the old lineup/cache before applying the new lineup; this prevents a retained 6-player lineup from filling empty HU seats. Filter changes MUST NOT delete a departed player's cache either; when that `playerId` is absent from the current lineup and cannot be recalculated, the last calculated snapshot remains visible. A spectator `EVT_DEAL` has no `Player` and therefore no trustworthy hero-anchored seat rotation, so it MUST NOT replace the retained lineup. **Seat turnover is immediate and cannot be masked by the cache**: any trusted lineup entry with a real (non `-1`) `playerId` always overwrites that seat's cache entry outright -- covers a brand-new player taking the vacated seat (MTT/cash reseating) and the same player returning (rebuy/reconnect) identically, per `SeatUserIds` alone (no cross-hand identity inference). The existing multi-seat conflict heuristic still clears the cache on a trusted table change when no explicit session boundary was observed. Import/batch-refresh lineups (`latestStats` chrome message) bypass the dim cache/mute state entirely -- they're a one-shot DB recompute, not the live per-hand pipeline. The cache itself is React memory and is not restored after a content-script reload; recent-hand records remain in IndexedDB.
- **Last-table restore** (sola要望 2026-08, `src/utils/last-table-storage.ts`): the #358 retention (`dimCacheRef`, the retained lineup, `lastKnownStats`) lives only in React memory, so an extension reload or browser restart used to wipe it — leaving post-session review impossible until the next live deal. `App.tsx` now persists the **displayed** non-hero lineup and restores it on mount as dimmed (離席-style) panels, so an opponent's stats and the recent-hands drill-down (an IndexedDB read keyed by `playerId`) are reachable immediately after a reload. Constraints, all load-bearing:
  - **Display-only** (MUST): the restored lineup MUST NOT feed the statistics pipelines, ACTIVE-port token logic, or replay gating. It is a record of what was rendered, not a claim that anyone is seated.
  - **Live is always authoritative**: the first applied live aggregate lineup discards the restored seats (so a genuinely empty seat shows "Waiting for Hand..." or *this* table's dim cache, never the previous table's player); the #361 session-start boundary discard applies to it exactly as to a live-retained lineup; a spectator `EVT_DEAL` (#359) does not overwrite it; and restoring only ever fills seats that are currently `playerId === -1`.
  - **Hero seat (display index 0) is neither saved nor restored** (MUST NOT): that panel belongs to the pre-game career-stats path below, and a restored older value could overwrite a freshly recomputed one.
  - **One write point**: the live per-hand `handleStatsMessage`, debounced 500ms (the same cadence as `PokerChaseService.persistState()`; the debounce coalesces same-tick bursts, it is not a throttle — per-action `realTimeOnly` updates never reach it). The `latestStats` batch path deliberately does NOT write: its pre-game variant is a hero-only array that would erase the record it was just restored from. A snapshot with no non-hero player is never written, so a boundary discard cannot blank the record.
  - **Fail-closed and versioned**: `LAST_TABLE_SNAPSHOT_VERSION` plus a Zod schema; a version mismatch, a type mismatch, or a duplicate seat index restores nothing (empty seats, as before) rather than restoring a partial record. `playerId` and `playerName` are stored explicitly alongside `statResults` because the drill-down導線 depends on the identity, not on the stat shape.
  - **Storage access goes through the background** (MUST): `chrome.storage.local` is `setAccessLevel('TRUSTED_CONTEXTS')`-restricted (#274) so a content script's direct `get`/`set` always rejects. The `getLastTableSnapshot` / `setLastTableSnapshot` message pair in `message-router.ts` is the only door, exactly like `getDeviceUILayout` / `setDeviceHudPosition`; the background re-validates on write so an unvalidated value is never stored.
...
- **Recent hands drill-down** (`getRecentHands`, `src/services/recent-hands-service.ts`, `src/components/hud/RecentHandsPanel.tsx` + `RecentHandsPanelTrigger.tsx`): HM3/PT4 "Last Hands" + Hand2Note "recent showdown hole cards" pattern, cloning the `#128` positional drill-down's architecture (indexed `hands`/`actions`/`phases` queries batched by `handId`, 30s cache keyed on `playerId`+filters (NOT `limit` -- see below), chevron trigger next to it in `HudHeader.tsx`, App-level `openPanel: { playerId, kind: 'positional' | 'recentHands' } | null` state making the two drill-downs mutually exclusive). Lists the player's last N hands (default 25, own `limit` param -- independent of `handLimitFilter`, which only bounds the aggregate stats), newest hand-id first. Key design points:
  - **Hole cards: OPPONENTS from the `Hand` entity, HERO from the Lake**: for every player other than hero, visibility is derived entirely from the already-persisted `Hand` entity and never touches `apiEvents` -- `hand.results` (persisted straight from `EVT_HAND_RESULTS.Results` by `write-entity-stream.ts`/`entity-converter.ts`) already carries each result row's `HoleCards`, and the server itself only ever sends valid card indices for cards that were actually shown, so the gate is `isShowdownParticipant(result)` (RankType 0-9 or 11 SHOWDOWN_MUCK) AND the `HoleCards` array actually holding valid values. RankType 10 NO_CALL and 12 FOLD_OPEN never show cards even though the server does send real values for a voluntary post-fold reveal (12) -- for opponents this panel is specifically "recent *showdown* hole cards". **Hero's own row is the deliberate exception** (#353): hero's dealt cards are hero's information from the moment they were dealt, they exist nowhere in the `Hand` entity, and they are read from the Raw Event Lake at read time -- see "Hero's own dealt hole cards" below. Do not "restore" a no-`apiEvents` invariant over that path; the two rules coexist by design and are split on whose cards they are. **The opt-in replay import adds a third source** (`holeCardsSource: 'replay'`, `src/replay/hole-cards.ts`, reading the `replayDetails` projection — not `apiEvents`): it fills only rows the `Results` path left blank, and it MUST NOT gate on `isShowdownParticipant`. While the game's card-open feature (〈手札公開機能〉) is active, `/replay/detail` carries `HoleCardList` for every seat recorded in the hand replay, and a seat that folded has no `Results` row at all — so a RankType-based gate is structurally unable to reach it. Which seats are disclosed is decided by the payload, which returns an empty / `-1`-filled array for anything it does not disclose; the reader's exactly-two-valid-cards check is the only filter.
  - **Preflop-line taxonomy** (`derivePreflopLine`, full doc comment on `PreflopLine` in `src/types/stats.ts`): a per-hand label in the compact shorthand common to HUDs/trackers (#356) -- `OR` open raise, `3B`/`4B`/`NB` bets, `CC`/`3CC`/`4CC` cold-calls **split by what was faced** (flatting an open and flatting a 3bet are different actions), `C` non-cold call, `L` limp, `X` check, `F` fold, `W` walk. Derived from the player's own PREFLOP actions plus a locally-recomputed `phasePrevBetCount` (base formula as in `write-entity-stream.ts`, replayed over a batched `actions.where('handId').anyOf(handIds)` fetch covering all seats -- own actions alone can't tell you what bet count you faced). Prior actions MUST be counted by their **effective** type (`resolveEffectiveActionType`), not the stored one: a short all-in that cannot cover the facing bet is stored as `RAISE` but is really a call, and counting it raw would misread `open -> short all-in -> call` as facing a 3bet (`CC` mislabeled `3CC`, and a subsequent raise's `3B` mislabeled `4B`). The label reflects the *last* action taken; if that's a FOLD and there was a preceding line, it gets a `-F` suffix (e.g. `3B-F`), which MUST stay `-F` rather than `/F` because `/` is the street separator in the postflop column. Note the cold-call family does NOT add one the way the bet family does: `phasePrevBetCount` counts the bets the action FACED, so facing 2 is facing the open (`CC`) while raising over 2 is a 3bet (`3B`). These labels are recomputed on every read and persisted nowhere (not IndexedDB, not `chrome.storage`, not any export or cloud upload), so the vocabulary can change without a migration.
  - **Hand count switcher & cache granularity** (#341): `RECENT_HANDS_LIMIT_OPTIONS` (10/25/50/100, `src/utils/recent-hands-config.ts`) drives a compact switcher in the panel header; the choice persists in `chrome.storage.local` (device-local, like `uiScale` -- `storage.sync` has per-minute write quotas a switcher trivially trips). Reads and writes are routed through the trusted background (`getRecentHandsPanelConfig`/`setRecentHandsPanelConfig` in `message-router.ts`, same as `uiScale`/`hudPosition_*`): the local area is closed to content scripts by `setAccessLevel('TRUSTED_CONTEXTS')` (#274), so the panel MUST NOT touch `chrome.storage.local` directly -- direct access rejects in a real browser and `storage.onChanged` never fires in untrusted contexts (the original direct-access implementation silently reset both #341/#353 choices on every page load; a regression test in `recent-hands-config.test.ts` pins the no-direct-storage rule). Every open panel follows changes through a background broadcast (`updateRecentHandsPanelConfig`, sent to game tabs after a successful persist, fanned out by `content_script.ts` as a window event -- the `updateHandLogLayout` pattern). The service always assembles and caches up to `MAX_RECENT_HANDS_LIMIT` entries and satisfies the caller's `limit` by slicing, so the cache key deliberately omits `limit`: switching counts costs no extra DB round-trip and does not fan the cache out into four copies of the same data. The constants live in a leaf module rather than in the service because the content-script panel needs them and MUST NOT pull Dexie/`background/replay-import` into its bundle. No virtualization: 100 rows x 6 cells is a fixed, small DOM (HandLog uses `react-window` because its row count is unbounded); the panel caps its body height and scrolls; since #357 there is no header row at all (the column legend and units live in cell tooltips), so nothing is sticky.
  ...
  - **Bet-size arithmetic — the two WS semantics that make it work** (#354). Both were verified against raw captures rather than assumed, and a change here MUST re-verify rather than reason from the field names:
    ...
    3. A stored `RAISE`/`BET` carrying `ActionDetail.ALL_IN` is **not necessarily aggressive**. The write path normalizes a raw `ActionType.ALL_IN` from `Progress.NextActionTypes` alone — if `CALL` is on offer it becomes `RAISE` *without comparing amounts* — so a short all-in that cannot cover the facing bet is stored as `RAISE`. `resolveEffectiveActionType` re-derives the real type for `ALL_IN`-flagged rows by comparing the actor's street-cumulative `bet` against the largest prior cumulative `bet` by another player on that street (seeded with `hand.bigBlind` preflop, where blinds never arrive as `EVT_ACTION`), and only keeps BET/RAISE when it strictly exceeds. Equal-or-under is a call, matching `hand-log-processor.ts`'s `ALL_IN` branch exactly (`BetChip > prevBet` alone prints "raises … and is all-in"; `===` and `<` both print "calls … and is all-in"). Rows without the `ALL_IN` detail MUST NOT be reinterpreted — the server sent that type explicitly.
  ...
  - **Preflop sizing** (`derivePreflopLineAmount` + `formatPreflopLine`, #354/#356): the raise-TO / call-TO amount is carried as `preflopLineAmountBB`/`preflopLineAmountChips` (preflop `Action.bet` is the street-cumulative total, which for preflop *is* the raise-to — blinds are already inside it) and rendered inline on the labels that carry one: `OR2.2`, `3B9`, `4B22`, and (since #356) the cold-call family's called-TO amount `CC2.2`, `3CC9`. `L`/`C`/`X`/`F`/`W` never get a number. The `-F` suffix composes **after** the number (`OR2.2-F`, `3CC9-F`) — it describes the outcome, not the label, so the size belongs to the label body. Exact chips stay in the cell's `title`. The number is kept out of the `preflopLine` string itself and composed at render time: that string is a documented, separately-tested taxonomy, and splicing digits into it would couple the taxonomy to display formatting.
    Two ways the number is deliberately suppressed rather than guessed: a label that carries no amount (`L`/`C`/`X`/`F`/`W`) never gets one, and `derivePreflopLineAmount` returns `null` when the amount is not real. Since #356 it selects the action that PRODUCED the label (`derivePreflopLabeling` hands the labelling action out of the same loop that builds the label, so the classification is never duplicated) and nulls the amount when `resolveEffectiveActionType` reclassifies that action as a call. The label reflects the player's last action, so the inline number must belong to that same action; returning an earlier open's size would print one action's number against a different action's label.
  ...
  - **Board column** (`deriveBoard`, #356): the hand's community cards, read from the already-batched `phases` rows -- `Phase.communityCards` is cumulative per street, so the longest array in the hand is the final board, which also covers all-in runouts where `EVT_DEAL_ROUND` never arrives and the board only exists on the synthesized FLOP/SHOWDOWN phases. No new query. It is the hand's board, not anyone's hole cards, so no visibility gate applies. Rendered fully run together with no street separators (`8962A`) -- sola's call; the street-separated notation lives in the cell's tooltip (`8h 9h 6h | 2s | Ad`), which is also the only place the full rank+suit appears.
  - **Rank-only cards** (`cardRankLabel`, #356): the board and hole-card cells render the RANK only and carry the suit purely in the 4-color coding, because 240px cannot fit rank+suit for a 5-card board alongside every other column (still true at #357's 9px: suit letters roughly double both card columns, ~+34px against ~26px of spare width). Each such cell MUST expose the full `R s` notation in its `title` (`ボード: 8h 9h 6h | 2s | Ad`) -- rank-only display relies on color alone, which is a real loss for anyone who cannot separate the four hues, and the tooltip is the only fallback. `T` stays `T`.
  - **Display conventions** (#353, from sola's DEV-build feedback): no time column (時刻は不要); results are shown in big blinds (`formatNetBigBlinds`, signed one decimal) divided by **that hand's own** `bigBlind`, because SNG/MTT levels escalate and raw chips are not comparable across a list — the raw chip amount stays as the cell's `title` rather than a second column, and a row whose `bigBlind` is unusable falls back to chips instead of being hidden; the F/T/R column separates streets with a spaced `/` (`XC / B33 / B66!`) while tokens inside a street stay run together, and uses tabular numerals. The table is `table-layout: fixed` with explicit `COLUMN_WIDTHS` (#356): with `auto` layout a single extreme row widens the shared columns and wraps *every other row* -- measured in Chrome, adding one worst-case row took wrapped rows from 2 to 5. Verify layout changes here by rendering in a real browser and measuring, not by estimating character widths: a character-width model under-predicted this and passed a layout that visibly wrapped, so the size digits do not make the column width jitter row to row, and the street cell is the one allowed to wrap rather than be clipped; hole cards use a four-color deck (`suitColor`/`SUIT_COLORS` in `card-utils.ts`, spades default / hearts red / diamonds `#4da6ff` / clubs `#66cc66` — lightened so they hold up on the translucent-black panel).
  - **No header row; columns sized from values alone** (#357): there is no `<thead>`. sola: 「ヘッダー説明文字列要らないんじゃないかな。見ればわかるし」. Removing it is not only visual -- with headers present, **Pos, カード and 損益(BB) were header-label-bound**, i.e. the Japanese label was wider than any value the column could hold, and that dead width sat inside the column as a gap. `カード` was the worst (header 21.7px vs 9.0px for two rank glyphs at 9px) and produced the カード↔ライン gap sola asked to close. With the header gone every column is value-bound, which is what pays for the body font going 8px -> **9px inside the unchanged 240px HUD** -- the panel is NOT widened.
    Measured in Chrome 151 at 9px monospace, worst-case value + 1px padding per side: Pos 15.5 (`BTN`/`UTG`) / カード 11.0 (two ranks; `—` is the same width) / ライン 42.5 (`3CC22.5-F`) / ボード 23.0 (`8962A`, letter-spacing -0.3px) / F/T/R 75.1 (`B150 / R80 / B66!`, letter-spacing -0.2px) / 損益 34.8 (`+122.5`) = **201.9px**, against 228px of table (240px minus the panel's 12px padding) and 213px where the scrollbar consumes layout width. `COLUMN_WIDTHS` are the *proportions* of those needs, so the leftover is shared evenly instead of piling into one gap, and no column starves at either scrollbar state (rendered at both: 0 wrapped rows, 0 overflowing cells). Two things a width model gets wrong here and a render does not:
    - **The 損益 column MUST be measured bold.** `won`/`lost` apply `fontWeight: bold`, so `+122.5` occupies 32.8px, not the 27.0px of the normal face. A distribution derived from the unbolded width overflowed that column (34px of content in a 32px cell) in the narrow-scrollbar case.
    - **Budget for a classic scrollbar (15px), not macOS's overlay scrollbar (0px).** The `scroller` always scrolls at these row counts, so sizing against the development machine's 0px scrollbar ships a layout that only wraps on users' machines.
  - **Where the header text went** (#357): the labels are gone, and nothing visible replaced them -- the columns' meaning has to come from the cells. Two `title` strings therefore carry information they did not before, and MUST keep carrying it: `formatPostflopTooltip` always returns the F/T/R notation legend (`X=チェック B=ベット …`, previously the F/T/R header's `title`) even for a row with no aggressive action or no postflop at all, and `netChipsTooltip` names the unit (`損益 +6.2BB（+1,240チップ、BB=200）`) and is now also emitted for rows that fell back to chips, which previously had no tooltip at all. Do not "simplify" either back to returning `undefined` on the empty case: that is the only path left to the notation and the unit.
  - **No showdown marker** (#357, sola指定): the 損益 cell used to carry a yellow `●` for `wentToShowdown`. Gone from the display -- whether a hand reached showdown is already readable from the cards and F/T/R columns, and at 8px it was the one cell that measurably overflowed its column (`+122.5●` needed 40px in a 37px cell). `wentToShowdown` stays in the data: `isVoluntaryParticipation` (the 参加のみ filter) reads it.
...
- **RealTimeStatsStream** - One ACTIVE-token-bound stream for real-time pot odds, SPR, and hand improvement; a different tab/document generation MUST reset its in-hand state before the new event enters it, while a bounded same-content-script reconnect preserves it
- **HandLogStream** - Hand history generation
...
- **ALL_IN normalization**: A same-street CHECK option MUST NOT become CALL: it is RAISE preflop (the BB option, including a shove below the minimum raise), and BET postflop (including a shove below the minimum bet). Preserve short/equal calls with only FOLD/ALL_IN available and the existing street-opening BET rule. See `docs/api-events.md` "EVT_ACTION: CHECK権からのALL_IN". Changes to these write-time classifications require Raw Event Lake rebuilds; advisory version 8 covers this correction.
...
- **3-Bet (3B)**: Preflop actions facing `phasePrevBetCount=2` are opportunities unless the previous nonempty `Progress` menu for that same seat and street proves no raise is available. RAISE or ALL_IN with CALL/CHECK permits a preflop raise; FOLD/CALL and FOLD/ALL_IN do not. An actual normalized RAISE MUST retain both numerator and denominator even if its menu disagrees; missing, empty, different-seat or different-street menus MUST remain unknown rather than denying the opportunity. This does not change how betting levels are counted.
- **3-Bet Fold (3BF)**: PT4's **general** "Fold to 3-Bet" variant — fold rate when facing curPrevBetCount=3, regardless of whether this player made the original raise being 3-bet ("cold-facing" is included). This is distinct from PT4's separate "Fold to 3-Bet After Raising" stat, which HUD does not implement. These are folding opportunities, so raise availability MUST NOT gate them.
...
| `statHandContributions` | `[generation+playerId+handId]` | generation/player/filter/latest-window composites | Bounded per-hand contributions for exact latest-N statistics (at most 500 rows per player × battle type × table-size cell) |
| `statPlayerAggregates` | `[generation+playerId]` | `generation`, `playerId` | Ready-state all-history counter aggregates and filter buckets |

v3 added composite indexes for player-specific queries. v6 changes the Raw Lake primary key to `[timestamp+ApiTypeId+sequence]`. v7 adds the `replayDetails` store. v8 adds the two statistics-ledger stores without eagerly scanning existing history during the versionchange transaction: the first HUD read lazily builds only the displayed players, and later completed hands update their counters in the same transaction as `hands`/`phases`/`actions`. Filtered latest-N reads remain exact because each tagged battle-type/table-size cell retains its newest 500 contributions. Neither schema v7 nor v8 changes canonical write-time entity derivation. The separate ALL_IN normalization and 3bet opportunity correction advances `REBUILD_ADVISORY_VERSION` to 8. Since IndexedDB cannot change an object-store primary key in place, v4 copies old rows to a staging store with `sequence: 0`, v5 drops the old store, and v6 recreates/copies it transactionally. Existing rows were already unique under the old key, so this migration is mechanical and does not change `hands`/`phases`/`actions`.

Every live raw `EVT_HAND_RESULTS` row MUST atomically create a pending-derivation fence keyed by its exact `[timestamp+ApiTypeId+sequence]` primary key. The exact fence MUST remain until either the canonical entities and statistics ledger commit together, or the pipeline conclusively rejects that result as a documented non-derivation terminal case. A fence owned by the current Service Worker boot is normal in-flight work; a foreign-owner, failed, or malformed fence MUST block a new lazy baseline and trigger Raw Lake recovery. After a live canonical transaction failure, the same worker MUST schedule non-blocking Raw Lake recovery using the exact fence ID as in-memory evidence even when persisting the fence's failed state also fails; successful activation MUST clear only that explicit fence ID. Chunked cloud rebuild activation MUST delete only the exact interrupted-fence IDs captured at rebuild start so concurrent live results survive; a manual full rebuild MAY clear every pending fence only while its transaction holds the `apiEvents` read-write lock and has replayed that complete Lake snapshot.
...
- **`storage.local`**: Device-local layout (`uiScale`,
  ...
  (`pokerChaseServiceState` — playerId, latestEvtDeal, session) and the
  display-only last-table snapshot (`lastTableSnapshot`, see "Last-table
  restore" under UI & Display). Layout access
...
- **Data Structure**: existing `/users/{userId}/apiEvents/{timestamp_ApiTypeId}` and `{timestamp_ApiTypeId_sequence}` documents remain readable. New events use `{timestamp_ApiTypeId_h_sha256}`, hashing canonical content without the local `sequence`. Uploads MUST confirm matching content or create with `currentDocument.exists=false`, never overwrite a different payload based on its locally allocated sequence. A legacy/content duplicate may remain after local sequence reassignment; the Raw Lake content-deduplicates it, and subsequent uploads MUST NOT keep multiplying it (see `docs/architecture.md`).
- **Content-ID rollout gates**: the owner MUST apply the API-event immutable-payload rules before releasing the new writer; rule merge/emulator success is not a production deployment. Old clients retain owner read/create/delete and same-content/sequence-only updates, but different-payload upserts are rejected. The companion poker-warehouse content dedup/sequence-collision change MUST also be merged and applied through `run.yml` before or alongside production content-ID writes. HUD-local dedup does not prove warehouse correctness. See `docs/architecture.md` for the rollout boundary.
