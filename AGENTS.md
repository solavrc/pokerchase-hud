...
- **Recent hands drill-down** (`getRecentHands`, `src/services/recent-hands-service.ts`, `src/components/hud/RecentHandsPanel.tsx` + `RecentHandsPanelTrigger.tsx`): HM3/PT4 "Last Hands" + Hand2Note "recent showdown hole cards" pattern, cloning the `#128` positional drill-down's architecture (indexed `hands`/`actions`/`phases` queries batched by `handId`, 30s cache keyed on `playerId`+filters (NOT `limit` -- see below), chevron trigger next to it in `HudHeader.tsx`, App-level `openPanel: { playerId, kind: 'positional' | 'recentHands' } | null` state making the two drill-downs mutually exclusive). Lists the player's last N hands (default 25, own `limit` param -- independent of `handLimitFilter`, which only bounds the aggregate stats), newest hand-id first. Key design points:
  ...
  - **Rank-only cards** (`cardRankLabel`, #356): the board and hole-card cells render the RANK only and carry the suit purely in the 4-color coding, because the panel cannot fit rank+suit for a 5-card board alongside every other column (still true at the #357 width: adding suit letters roughly doubles both card columns, ~+45px on top of the 292px panel). Each such cell MUST expose the full `R s` notation in its `title` (`ボード: 8h 9h 6h | 2s | Ad`) -- rank-only display relies on color alone, which is a real loss for anyone who cannot separate the four hues, and the tooltip is the only fallback. `T` stays `T`.
  ...
  - **Panel width is decoupled from the HUD's 240px** (`RECENT_HANDS_PANEL_WIDTH_PX`, #357): the drill-down renders at **292px with a 10px body font**, the same size as the rest of the HUD (`Hud.tsx`'s container font and the player name are both 10px) -- #356's 8px was the one place in the HUD that had shrunk to fit the 240px shell, and sola asked for it back («文字サイズ上げられません？»). Measured in Chrome 151 at 10px monospace, the six columns need 245.8px total (Pos 19.0 / カード 26.1 / ライン 47.0 / ボード 26.5 / F/T/R 83.6 / 損益(BB) 43.6, each the larger of its header label and its worst-case value, +1px padding per side); Pos, カード and 損益(BB) are **header-bound**, so shortening values there buys nothing. The outer width adds 2px border + 12px padding + **15px for a classic scrollbar** -- the `scroller` always scrolls, and macOS overlay scrollbars are 0px while Windows/Linux Chrome takes real width, so the budget MUST be computed for the wider case or the layout only wraps on users' machines. `COLUMN_WIDTHS` are the *proportions* of those measured needs, so every column keeps its required width at either scrollbar state (verified by rendering at 292px and at 277px = the 15px-scrollbar equivalent: 0 wrapped rows, 0 overflowing cells in both).
    Two mechanics this depends on, both of which a render caught and a model would not have:
    - `Hud.tsx` sets `overflow: visible` on the HUD body **only while this panel is open** (otherwise the wider panel is clipped away by the `overflow: hidden` that keeps children inside the rounded corners). The panel therefore carries its own background/border/radius -- the parent's background is only 240px wide, so without them the overhang would be text floating on the game.
    - The expansion direction comes from `resolveRecentHandsPanelAnchor(left)`: a HUD in the left half of the screen aligns its **left** edges and grows right, one in the right half aligns its **right** edges and grows left, so the panel's outer edge never passes the HUD's own outer edge and the panel is on-screen exactly when the HUD is (expanding symmetrically loses that guarantee; at 1280px wide the default `left: 90%` seats would hang ~22px off). The direction is applied with a flex wrapper (`justify-content: flex-start`/`flex-end` + `flex-shrink: 0`), **not** negative/auto margins: measured in Chrome, `margin-left: auto` + `margin-right: -1px` is over-constrained and CSS 2.1 §10.3.3 collapses the `auto` to 0, so the panel grew rightward off-screen instead of aligning right; and without `flex-shrink: 0` the flex container shrank the 292px panel back to 241px.
  - **No showdown marker** (#357, sola指定): the 損益(BB) cell used to carry a yellow `●` for `wentToShowdown`. It is gone from the display -- whether a hand reached showdown is already readable from the cards and F/T/R columns, and at 8px the marker was the one cell that measurably overflowed its column (`+122.5●` needed 40px in a 37px cell). `wentToShowdown` stays in the data: `isVoluntaryParticipation` (the 参加のみ filter) reads it.
