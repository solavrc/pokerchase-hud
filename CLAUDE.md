...
- **Side pot handling**: `collected X from main pot` / `from side pot` / `from side pot-N` (PS準拠)。勝者判定は `HandRanking` ベース + `RewardChip` フォールバック。メインポット勝者がサイドポット対象外（アンテオールイン）のケースあり。`Pot + sum(SidePot) == sum(RewardChip)` 不変条件に依存。
- **Ante all-in chip estimation**: `Chip=0, BetChip=0` のプレイヤーが複数いる場合、`Progress.Pot/SidePot` の段差から拠出額を逆算（`buildAnteAllInChipsMap`）。`EVT_HAND_RESULTS.RewardChip` で正しい座席に割り当て（`fixAnteAllInChips`）。座席番号 ≠ スタック順。
- **BB action skip**: PokerChase は全員オールイン/フォールド時に BB のアクションをスキップする。`getMissingBBCheck` で `checks` を補完（`NO_CALL` 勝利時は除外）。
