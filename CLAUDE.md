...
- **Duplicate Prevention**: reconnect resends are identified by canonical payload content (ignoring storage-only `sequence`/`arrivalOrder`), not by timestamp+ApiTypeId alone. Distinct payloads sharing the same millisecond and type are both retained.
...
- **Equal-millisecond replay**: primary/cloud identity stays `[timestamp+ApiTypeId+sequence]`. Live ingestion adds background-global `arrivalOrder` before the async durability queue. Stateful readers use `processInReplayChunks()` or `orderApiEventsForReplay()`, holding equal-ms groups across primary-key page boundaries. Legacy rows apply only the observed 305→first same-phase 304 causal rule; never treat primary-key/NDJSON line order as original arrival order.
