...
## Replay import disclosure（v6.0 公開時）

リプレイ取り込みは既定OFFのオプトイン機能で、有効化するとPokerChase自身の
リプレイAPIへリクエストを出す。外部への送信ではない（PokerChaseのサーバのみ）
が、**拡張が能動的に発生させる通信**なので、公開前に次を済ませる。

1. Chrome Web Store の説明と Privacy practices に、オプトイン時のみ・
   セッション外のみ・プレミアムパスの有効期間の検証後のみ・自分が参加したハンドの詳細を
   PokerChase のサーバから取得して端末に保存する旨を明示する。保存される詳細に
   は、ゲームの〈手札公開機能〉が公開する範囲でリプレイに記録された全プレイヤー
   の手札（フォールドした手札を含む）が含まれることも書く。既定はOFF。
2. 更新情報（GitHub Release / ストアの更新履歴）で、既存ユーザーにも新機能と
   その既定OFFを明示する。公開トグルON後、`/replay/list`を1回送って
   `IsExpiredCardOpen`と`CardOpenEndDate`を確認する。対局中または認証
   エンベロープ未捕獲なら通信せず保留する。
3. [privacy policy](../PRIVACY.md) の "Optional replay import" 節が公開版に
   反映されていることを確認する。新しい host permission は追加しない
   （ゲームのページ内から、ゲーム自身と同じオリジンへ出す）。
4. 取得できるのは当日と直前3暦日ぶんに限られる（サーバ側の制約）ことを、
   期待値の齟齬を避けるため説明文にも書く。
5. 取り込みサイクルごとに同じ期限フィールドを再確認し、失効時は新規取得を
   停止するが、取り込み済みデータは削除しないことを明示する。

### ストア掲載文（下書き）

> リプレイ取り込みを有効にすると、自分が参加したハンドについて、ゲームの
> リプレイ機能が返す詳細を保存し、リプレイに記録された全プレイヤーの手札
> （フォールドした手札を含む）を直近ハンドに表示します。公開される範囲は
> ゲームの〈手札公開機能〉が決めます。プレミアムパスの有効期間内のアカウントで
> 利用できます。
>
> 利用状態の確認と詳細取得は対局中には行わず、対局終了後に実行します。
> 認証情報がまだ取得できない場合はホーム画面を開くまで保留します。対象期間は
> 当日と直前3日です。既定はオフで、失効後も取り込み済みデータは残ります。

### 更新情報の見出し（下書き、sola推敲前）

> リプレイ取り込み: プレミアムパスの有効期間を確認してリプレイ詳細を保存します（既定オフ）
