...
実データ（393,830イベント）で8ハンド、同一HandIdに対しFLOPの `EVT_DEAL_ROUND`（CommunityCards 3枚）が異なるコミュニティカード・異なる残存プレイヤー構成で2回配信される事例を確認した。公式のゲーム仕様では説明できない未解明の挙動（run-it-twice相当の何かの可能性はあるが未確認）。

- `Phase`（エンティティ側）は `[handId, phase]` を主キーとして重複排除されるため、DB保存上は問題を起こさない
- `npm run verify-stats` のオラクル（`src/tools/verify-stats/oracle.ts`）はハンドあたり単一のフラットなコミュニティカード・生存者スナップショットしかモデル化できないため、この事例を再現できない。WTSD/WWSF/CBetの一致率が100%に届かず99.5〜99.7%付近で頭打ちになる既知の無害なギャップの原因となっている（ライブパイプライン側は`EVT_DEAL_ROUND`ごとに`Phase`を保存するため正しく処理できている）
- 詳細: `CONTRIBUTING.md`「WTSD/WWSF ≈ 99.5–99.7%」、`src/tools/verify-stats.ts` のヘッダコメント
...
| デュアルボード観測（未解明） | `EVT_DEAL_ROUND` | 同一HandIdでFLOPが異なるカード・異なる残存プレイヤー構成で2回配信される事例（実データ8ハンド）。公式仕様で未説明。`verify-stats`オラクルは再現不可（[詳細](#デュアルボード観測原因未解明)）。 |
...
| 6 | 自発退出 | 0.02% | Chip>0を残したまま次ハンドでGONE。Ringの途中退出。Status=1から遷移するパターンあり |
