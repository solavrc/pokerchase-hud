...
- 唯一の例外がhand境界の303/306同居である。303 < 306 のため保存順では必ず配札が
  終了行を追い越すが、1ミリ秒で配札から決着までが完結することはあり得ないので、
  その306は必ずgroupより前に始まったハンドの終了行である。stateful readerは
  EVT_DEALでハンドバッファを確定させるため、この倒錯を放置すると
  「前ハンドはHandId未設定で破棄」「次ハンドのバッファが前ハンドのHandIdで確定」
  「次ハンドの残り行はバッファ不在で消失」と、1つの倒錯で2ハンドが失われる。
  したがってgroup内の306は最初の303より前へ戻す。同じgroupの304が前ハンドの
  最終アクションか次ハンドの初手かは決められないため、主キー順のまま303の後ろに
  残す（最悪1アクションの誤帰属で、ハンド消失は起きない）。303を含まない
  306群（実キャプチャで多い306+311 / 306+309 / 306+313）は一切触らない。
- 実raw 393,830 events（210 group）と 561,309 events（21,581 group）のどちらにも
  ...
  ときにだけ起こる（スロットル/凍結されたタブの再開、reconnect resend、圧縮リプレイ）。
  ライブ経路は到着順で処理するため常に無傷で、失われるのはLakeを再生する経路
  （手動フル再構築 / import / hand-log export / verify-stats）だけである。
