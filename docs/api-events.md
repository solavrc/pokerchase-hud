...
現在のRaw Event Lake行にはtab/source attributionが保存されないため、再構築では309後の
DEAL単独をFriend SNG継続の十分条件にできない。201/308も欠落して識別不能な場合は、
終了済みsessionを復元せずbattle typeを未設定のまま扱う（fail closed）。
