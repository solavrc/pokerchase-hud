...
- HUD が認識している既知イベントには Zod スキーマが `src/types/api.ts` に定義されている。未知の `ApiTypeId` や既知typeのschema不一致payloadは Raw Event Lake にだけ保存される
- ランタイムバリデーションにより型安全性を確保
- 型ガード関数による安全なイベント処理を推奨

匿名化した実イベントの部分抜粋と、イベント間で値を復元する例は
[APIイベント実例集](api-event-examples.md)を参照。
...
| `EVT_SESSION_RESULTS` | 309 | セッション終了、クリーンアップ | `Ranking`, `IsRebuy`, `RankReward` / `RingReward` / `TournamentReward`（条件付き） |
...
`201`（参加申込）、`202`（アクション完了）、`203`（参加取消）、`205`
（タイムバンク）は、`Code=0`の成功応答だけでなく、非0の`Code`と
`Error { Status, Message, AddParam, Replaces }`を持つエラー応答にも同じ
ApiTypeIdが使われる。2026-07-29の実ログでは`201/Code=5205`と
`202/Code=5402`、2026-08の診断では`203/Code=5003`と`205/Code=5302`を
観測した。`311`（ハンド終了通知）では、既知の`NotifyCode=1/2/202`に加えて
`601`も観測した。HUDは311の通知コードで分岐しないため、非負整数を受理する。

これらは既知スキーマとしてRaw Event Lakeに保存する一方、非0 Codeの201を
セッション開始とは解釈しない。203は`Code=0`だけを参加取消の確定境界とし、
非0 CodeまたはCode欠落ではkeepalive、Service Workerのactivity、page-worldの
replay gateをinactiveへ倒さない。205と311を含め、ハンド履歴・統計には投入しない。
なお、201の`Code`自体が欠落する未知のschema変更時は、ゲーム中の誤reloadを
避けるため従来通りfail-closedでセッション開始シグナルとして扱う。

## 典型的なセッションのイベントシーケンス

セッション = 1ゲームの開始から終了まで（トーナメント1回分、リングゲーム1セッション分）。
以下は理解用の典型形であり、イベント回数を境界の不変条件として使ってはならない。

```
1. EVT_ENTRY_QUEUED (201)        ── セッション/テーブル着席
   - BattleType でゲームタイプ確定（SNG/MTT/Ring 等）
   - セッション内の全ハンドが同じ BattleType を持つ

2. EVT_SESSION_DETAILS (308)     ── ゲーム設定（欠落または再発行あり）
...
6. EVT_SESSION_RESULTS (309)     ── セッション終了（欠落・interleaveあり）
...
| ゲームタイプ | 特徴 |
|---|---|
| SNG (BattleType=0,2,6) | テーブル移動なし。EVT_PLAYER_SEAT_ASSIGNED は0または1回（欠落を許容）。2026-07-21 stagingでは0/2/6のsession coverageが733/971、1/10、7/19で、BattleType=2の1件は完走handのないentry-only区間だった。プレイヤー脱落で人数減少。 |
...
**重要**:
- `EVT_ENTRY_QUEUED` は「テーブルへの着席」ごとに発行される。SNG/Ring では通常1セッション1回だが、共有IndexedDBの複数タブinterleaveやcapture欠落では絶対境界にできない。**MTT ではテーブル移動のたびに再発行**される（同一トーナメントで複数回）。
...
|---|---|---|
| ランク戦SNG (0) | ルーム種別（`stage006_002` 等）。一意ではない。 | 最初の `HandId` をセッション識別子候補として使用。全プレイヤーが同一テーブルで開始するため、完全capture間では最初の HandId を共有する。 |
| Friend SNG (2) | 観測範囲では試合固有の数値文字列。 | `Id`を候補にできるが、複数タブinterleaveではobserver stream上の201/309だけで境界を確定しない。 |
| Club SNG (6) | `club_bt_...` 形式のクラブマッチID。 | `Id`とhand時系列を併用し、capture欠落を許容する。 |
...
**Friend SNG（BattleType=2）の境界例外（2026-07-21 staging）**: 201が1回の
区間に309が10回あり、各309後も新しい201なしでhandが継続するcaptureを確認した。
これは同一観測者の複数タブ/複数試合が共有IndexedDBへinterleaveした可能性があるため、
201や最初のHandIdだけで試合境界を確定しない。309、利用可能なら308、handの時系列、
tab/source attributionを組み合わせ、各境界シグナルの欠落も許容する。
...
EVT_DEAL 時点の `Chip` / `BetChip` は **アンテおよびブラインドが会計へ反映された後** の値。
...
開始スタックの逆算: `Chip + BetChip + Ante`（アンテを満額拠出した通常ケース）。ショートオールイン時は実際のアンテ拠出額へ置き換える。これは303の**会計内訳**からの復元であり、サーバーがアンテとブラインドをどちらから先に投稿処理したかはwireから観測できない。具体例は[APIイベント実例集の強制投稿オールイン](api-event-examples.md#303304306-強制投稿オールインとサイドポット)を参照。
...
- **強制投稿オールインプレイヤー**: アンテまたはブラインドの強制投稿で全チップを消費した場合、そのプレイヤー自身のEVT_ACTIONは送信されない。`Chip=0, BetChip=0` のアンテのみオールインと、`Chip=0, BetChip>0` かつSB/BB席のブラインドオールインは区別する。deal 時点の `BetStatus` はどちらも `BET_ABLE` / `ALL_IN` の観測例があり、必須判定条件ではない
- **タイムアウト / 切断**: 明示的な FOLD の EVT_ACTION が送信されないことがある。この場合、プレイヤーは EVT_HAND_RESULTS の Results にも含まれない
- **BB 強制投稿オールイン時の CALL**: PokerChase は BB がアンテと BB の強制投稿でオールインしても、SB に `CALL bet=BB額` を送信する（内部的に設定上の BB ベットが存在する扱い）。これは `Chip=0, BetChip>0` かつBB席で判定する**ブラインドオールイン**であり、`BetChip=0` のアンテのみオールインとは区別する。deal 時点の `BetStatus` は実データで `BET_ABLE` と `ALL_IN` の両方があり、必須判定条件にしてはならない
...
### EVT_ACTION: Progress.Phase はストリートの正典（ハンド終了行を除く）

`EVT_ACTION.Progress.Phase` は、そのアクションが実際に行われたストリートを payload 自身が持っている値であり、**保存順に依存しない**。`EVT_DEAL_ROUND` の到着で進めるカウンタは、以下の経路で実際のストリートより遅れることがある。

1. **同一ミリ秒バースト**（再接続時の一括再送とみられる）。主キー順 = `ApiTypeId` 昇順のため、304 が同時刻の 305 より前に並ぶ。`orderApiEventsForReplay` の strict predicate は孤立した2イベント群しか反転しないため、3件以上の複合群は保存順のまま残る（[Raw Event Lake のキー・順序・重複](#raw-event-lake-のキー順序重複)）
2. `EVT_DEAL_ROUND` の未受信（オールイン後の自動進行）
3. テーブル移動時のバッファ融合

**ただしハンド終了行（`Progress.NextActionSeat = -2`）では `Progress.Phase` が実際のストリートに関わらず 3 固定で届く。** 2026-08-01のエクスポート（40,932ハンド / 345,016アクション）で全数検査した結果:

| 対象 | 件数 |
|---|---|
| 終了行を除いた `Progress.Phase` とカウンタの不一致 | 29ハンド / 63アクション（0.07%） |
...
Ring（BattleType 4/5）では、ハンドの進行中に席のスタックが**増える**ことがある。買い足し（リバイイン／アドオン）とバイイン上限への自動復帰である。トーナメントでは発生しない。

流入は次の2箇所に現れる。

- **ハンド内スナップショット**: そのプレイヤーの `EVT_ACTION.Chip` や、次ストリートの `EVT_DEAL_ROUND.OtherPlayers[].Chip` が、直前の観測値から増える
- **終了スタックのみ**: `EVT_HAND_RESULTS` の `Chip` にだけ現れ、ハンド内スナップショットには一切現れないケースがある（ハンド終了時にバイイン上限へ戻す自動買い足し）

席ごとのスタックには次の不変条件があるため、流入額は決定的に復元できる。

- 同一ストリート内では `Chip + BetChip` は不変（ベットは `Chip` → `BetChip` の移動）
- ストリート境界では `Chip + BetChip` はちょうど直前ストリートの最終 `BetChip` だけ減る
- 終了スタックは「直近スナップショット − 残ストリート投入 + `RewardChip`」

2026-08-01のエクスポート（40,932ハンド）での実測: この流入によりディール→リザルトの卓合計が増えるハンドは157件、流入を差し引かないと拠出額が過小になるハンドを含めると176件。

> **注**: この不変条件からの**減少**は流入ではなく、観測列自体の破綻（融合バッファ、イベント欠落、下記「クロージングコールの欠落」等）のシグネチャである。
...
匿名化した実イベントJSONは[APIイベント実例集の終端BBブラインドオールイン](api-event-examples.md#303304306309-終端bbブラインドオールイン)を参照。
...
終盤12ハンドは `P4` と `P6` のヘッズアップで、問題の4ハンドはいずれもアンテのみオールインではない。この4ハンドで観測した303値は `BetStatus=ALL_IN, Chip=0, BetChip>0` で、SBまたはBB席の `BetChip` に正の短縮投稿額が割り当てられた**ブラインドオールイン**である。セッション全59ハンドには `BetChip=0` のアンテのみオールインは0件、ブラインドオールインは4件あった。ただし別の実データでは同じ `Chip=0, BetChip>0` のブラインドオールインでも `BetStatus=BET_ABLE` が存在するため、一般的な分類は座席・残チップ・投稿額から行う。
...
- `ResultType=1` は「ヒーロー敗退」を表す値ではない。全履歴のランク戦SNGでも同値327ハンド中、ヒーロー1位が177件、2位以下が150件で、対象#59もヒーローP6が1位だった。一方、既存実データfixture hand 421667480ではヒーローが6位で脱落しても `ResultType=0` である。`1` はトーナメント終了に伴う結果遷移と整合する観測ラベルだが、ヒーローの脱落/最終順位確定を網羅する判定値ではない。

#### parser / HUD 注意点とfixture候補

- `BetStatus=ALL_IN, Chip=0` だけで「アンテオールイン」と表示しない。`Chip=0, BetChip=0` はアンテのみ、`Chip=0, BetChip>0` かつSB/BB席はブラインド強制投稿オールインとして分ける。deal 時点の `BetStatus` は `BET_ABLE` の場合もあるため、分類の必須条件にしない。
...
- ヒーロー脱落・最終順位・セッション終了の判定は `ResultType` 単独ではなく、`Results[].Ranking`、`DefeatStatus`、終了スタック/Status、後続309を併用する。勝者にも `ResultType=1` が出る一方、早期脱落では `ResultType=0` も出る。
...
`ResultType=1` の表示名は**中程度の確信度**で「トーナメント終了遷移（観測）」とする。勝者/敗退者の双方で再現し対象セッションの309とも整合するが、早期脱落の `0` も存在し、サーバー内部の正式な列挙名も公開されていない。ヒーロー脱落シグナルとして使わず、「終了理由」や「報酬確定」まで意味を広げない。
...
| アンテのみオールイン | `EVT_DEAL` / `EVT_ACTION` | 303会計が `Chip=0, BetChip=0` のケース。blind席でも `BetChip` 割当ては0で、そのプレイヤーの `EVT_ACTION` は発行されない。`BetStatus` は実データで `BET_ABLE` / `ALL_IN` の両方があり、必須判定条件ではない。 |
| ブラインドオールイン | `EVT_DEAL` / `EVT_ACTION` | 303会計が `Chip=0, BetChip>0` かつSB/BB席のケース。`BetChip` に正の短縮SB/BB額が割り当てられ、そのプレイヤーの `EVT_ACTION` は発行されない。`BetStatus` は実データで `BET_ABLE` / `ALL_IN` の両方があり、必須判定条件ではない（[終盤ヘッズアップの実測](#終盤ヘッズアップの強制投稿オールイン2026-07-21-jst-観測)）。 |
...
| ハンド終了行の Phase 固定 | `EVT_ACTION` | `Progress.NextActionSeat = -2`（ハンド終了）の行では `Progress.Phase` が実際のストリートに関わらず **3固定**で届く（[詳細](#evt_action-progressphase-はストリートの正典ハンド終了行を除く)）。 |
| ハンド中のチップ流入 | `EVT_DEAL_ROUND` / `EVT_HAND_RESULTS` | Ringではハンド進行中に席のスタックが増える（リバイイン／アドオン／バイイン上限への自動復帰）。トーナメントでは発生しない（[詳細](#ring-ハンド中のチップ流入リバイインアドオン)）。 |
...
| ディール時のチップ値 | `EVT_DEAL` | `Player.Chip` と `Player.BetChip` はアンテ/ブラインドが**会計へ反映された後**の値。開始スタック = `Chip + BetChip + Ante`（満額アンテの通常ケース）。会計内訳から実際の投稿順は分からない。 |
...
|---|---|---|---|
| 0 | 通常結果 | 98.5% | 多くは次のハンドへ進むが、ヒーローが早期脱落して正の `Results[].Ranking` が付く場合もある。脱落判定は `ResultType` ではなく `Results[].Ranking` / `DefeatStatus` を使う |
| 1 | トーナメント終了遷移（観測） | 0.8% | 敗退だけでなく優勝時にも出現する（ランク戦SNG実測327件: 1位177件、2位以下150件）。全てのヒーロー脱落を表す値ではない。通常はEVT_SESSION_RESULTSが続くが、過去のスキーマ拒否期間には欠落あり |
| 2 | テーブル移動 | 0.4% | MTT。後続313を受信できた例では `ProcessType=1` / `2` の両方があり、313自体が欠落するcaptureもある |
...
- `apiEvents` の主キーは `[timestamp+ApiTypeId+sequence]`。`sequence` は同じ
  `timestamp`（クライアントの `Date.now()`）かつ同じ `ApiTypeId` 内の非負・一意な
  保存slotで、wire payload の一部ではない。liveの新規衝突は通常 `max(sequence)+1`、
  import / cloud復元は空いていれば指定sequenceを保持する。これにより同一millisecondに
  届いた同種の別イベントをどちらも保持できる。
...
| 制約 | 詳細 |
|---|---|
| EntityConverter の状態継続 | `convertEventsToEntities()` は1回の完結変換用。チャンク処理では**同じinstance**の `convertEventChunk()` を順番に呼び、最後に `flush()` する。chunkごとにconverterを作り直すと境界をまたぐハンドが失われる。クラウド復元はこの継続API、手動フル再構築は全件を1回で変換する。 |
...
| `STEAL` | スチール実行 |
| `STEAL_CHANCE` | スチール機会 |
| `FOLD_TO_STEAL` | スチールにフォールド |
| `FOLD_TO_STEAL_CHANCE` | スチールに直面（フォールド機会） |
| `RIVER_CALL` | リバーでコール |
