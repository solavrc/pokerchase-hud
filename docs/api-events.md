...
終盤12ハンドは `P4` と `P6` のヘッズアップで、問題の4ハンドはいずれもアンテのみオールインではない。厳密な分類は `BetStatus=ALL_IN, Chip=0, BetChip>0` の**ブラインドオールイン**（アンテ支払い後、SBまたはBBの強制投稿で残りスタックも投入）である。セッション全59ハンドには `BetChip=0` のアンテのみオールインは0件、ブラインドオールインは4件あった。

| セッション内hand | 分類 | P4の開始スタック → 強制投稿 | deal時 `Pot / SidePot` | 唯一の `EVT_ACTION` と決済 | P4の終了 → 次hand開始 |
|---|---|---|---|---|---|
...
各行の「開始スタック」は直前handの `EVT_HAND_RESULTS` 終了スタックと一致し、`Chip + BetChip + Ante` でも独立に再計算できる。この式を使えるのは、ここでP4がアンテ全額を支払った後にブラインドでオールインしているためである。アンテのみオールインとの反証比較では、別セッションの hand `530091830` が `Chip=0, BetChip=0`、deal時 `Progress.Pot=2,008`、アンテ拠出者4人であり、実際の短スタックは `2,008 / 4 = 502`（設定アンテ630未満）となる。両シグネチャを同一分類にしてはならない。

#### 推定ゲーム状態

- #55・#57・#59では、P4はBB席でアンテ後の残チップをBBとして全額投稿した。P4自身の `EVT_ACTION` はなく、P6（SB）に設定上のBB額までの `CALL` だけが送信された。
- #56では、P4はSB席で強制投稿オールイン済みで、P6（BB）は追加投入なしの `CHECK` を受け取った。独立した別セッションの hand `529819815` でも、SBブラインドオールイン後にBBの `CHECK` が観測されており、#56固有の偶発的な順序ではない。
...
セッションをhand単位ではなく連続状態として機械照合した結果は次のとおり。
...
- Pot/SidePotは必ず合算し、mainの勝者とsideの勝者を別々に決済する。#55（main split + side）、#56（SB all-in + BB check）、#57（callでside生成）、#59（終端順位 + `ResultType=1`）は相互に異なるfixture候補になる。raw payloadや実名/UserIdはfixtureへコピーせず、金額・席・イベント順だけを最小化して再構成する。
- セッション終了判定は `ResultType` 単独ではなく、`Results[].Ranking`、終了スタック/Status、後続309を併用する。勝者にも `ResultType=1` が出る。

#### 確信度・代替解釈

ブラインドオールイン分類、投入差分、main/side決済、P4の生存/脱落、最終順位は**高確信度**。rawイベント順、前後スタック、チップ保存、button/blind巡回、306/309の順位が同時に一致している。`BetChip>0` をアンテのみオールインとみなす代替解釈は、投稿額が毎回その席のSB/BB上限と一致し、相手のCALL/CHECKおよびPot増分まで説明できないため棄却する。
...
| アンテのみオールイン | `EVT_DEAL` / `EVT_ACTION` | `BetStatus=ALL_IN, Chip=0, BetChip=0`。アンテで全チップを消費し、そのプレイヤーの `EVT_ACTION` は発行されない。 |
| ブラインドオールイン | `EVT_DEAL` / `EVT_ACTION` | `BetStatus=ALL_IN, Chip=0, BetChip>0` かつSB/BB席。アンテ後の残チップをブラインドとして全額投稿し、そのプレイヤーの `EVT_ACTION` は発行されない（[終盤ヘッズアップの実測](#終盤ヘッズアップの強制投稿オールイン2026-07-21-jst-観測)）。 |
