...
**適用範囲（例外）**: この保存則は、セッション開始時の着席者構成がそのまま維持されているハンドに対して成立する。Status=6/7（自発退出/強制退出、下記「プレイヤー離脱状態」参照）でランク戦SNGのプレイヤーが `Chip` を残したまま `SeatUserIds` から恒久的に離脱するケース（残留チップがどこにも再配分されない）が観測されており、そのようなリタイア後のハンドでは離脱者の残留チップ分だけ左辺が `table_size × DefaultChip` を下回る。全ハンドに無条件で成立する式ではなく、リタイアを挟まない区間限定の保存則として扱うこと。
