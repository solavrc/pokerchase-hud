...
以下は#57の実イベントから必要フィールドだけを残した**部分抜粋（原文全文ではない）**である。フィールド名と値は改変せず、`SeatUserIds`、`UserId`、カード、他席などの不要な識別/ペイロードを省略した。`OtherPlayers` も該当する seat 3 のみに絞っている。
...
#57の開始スタック2,624はこの303内の直接フィールドではなく、直前#56の306終了スタックからの連続照合である。その値は303の `Game.Ante + OtherPlayers[seat 3].BetChip + Chip = 630 + 1,994 + 0 = 2,624` と一致する。
...
この306は `Pot + ΣSidePot = 5,248 + 506 = ΣRewardChip` を直接確認できる。匿名化前の `UserId ↔ SeatUserIds` 照合による表示上のP4/P6対応は抜粋外であり、rawにpot別受取人や `uncalled return` の独立フィールドはない。
