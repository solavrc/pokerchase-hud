...
`Progress.NextActionTypes` と現在のストリートを使う。同じストリートで
`CHECK` が可能ならコールすべき差額が無いため、そのオールインはCALLではない。
...
通常の`BET`選択肢ありはBET、`CALL`選択肢ありはRAISEという既存分類は変わらない。
選択肢が空のときはCHECK権を推定せず従来のCALLへのフォールバックを保持する。
ただし、そのアクション自身の`Progress.Phase`で新ストリート開始が確定する場合は、
...
空・別席・別ストリートのメニューは不明として従来の機会判定を保持し、
...
空メニュー・別席メニュー・前ストリートメニューの失敗注入も含む。
ベット段階の数え方は維持する。
