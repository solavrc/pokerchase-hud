...
補正値は「自分のランクポイントが同卓5人の平均よりどれだけ高い/低いか」で決まり、下振れ側（平均より600以上低い）で+14、上振れ側（平均より600以上高い）で-14、差が±40近辺で±0になる。

> **出典内部の矛盾（未解決）**: Wikiの計算例（STAGE Ⅲ・5位・平均より92低いケースで、基準値-15に補正+2を足して合計-13とする例）は5位の基準値を **-15** としているが、同じページの基準値テーブルはSTAGE Ⅲの5位を **-8**（-15は6位）としており、出典自体が内部で矛盾している。手元の観測データにSTAGE Ⅲのセッションは1件も含まれないため（観測されたのは stage000/002/004/005/006 とステージⅤ/Ⅵ表記のみ）、どちらが正しいかを実データで判定できない。下記の検証結果はこの矛盾の影響を受けない（STAGE Ⅲが母集団に存在しないため）。STAGE Ⅲのデータを扱う実装は、この2値のどちらでも成立するよう±14バンドを広めに取るか、実データ入手時に再検証すること。
