# 履歴資料: 全22ハンド詳細分析（#115 より前）

> **履歴資料（#115, 2026-07）**: この監査は2026-03時点の実装を対象にしたものである。
> その後の監査で、AF/AFq（ポストフロップ限定に変更）、WTSD/WWSF（PT4公式定義に
> 合わせプリフロップオールインを含めるよう変更）、VPIP/PFR（ウォーク除外の分母に
> 変更）の各定義がPT4/HM3の公式定義に合わせて再整合された。本ページに記載の
> AF/AFq/WTSD/WWSF/VPIPの値・判定根拠は**変更前の旧定義**に基づくものであり、
> 現在の実装の値とは異なる場合がある。最新の定義はCLAUDE.mdの
> 「Confirmed Statistical Definitions」を参照。この監査ドキュメント自体は
> 当時のハンド単位のトレース記録として残しており、現在の利用方法や期待値を確認する
> ための資料ではない。
...
| 統計 | 現在の実装 |
|------|------------|
| VPIP / PFR | BB ウォークを機会数の分母から除外 |
| AF / AFq | ポストフロップのアクションのみ集計 |
| WTSD / WWSF | プリフロップオールインでフロップが配られたハンドも既定統計に含む |
| WTSDa / WWSFa | プリフロップオールインを除外するオプトイン統計（既定は非表示） |
| W$SD | `SHOWDOWN` と `SHOWDOWN_MUCK` をショウダウンとして集計 |
