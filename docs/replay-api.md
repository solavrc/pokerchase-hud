...
判定は `update-manager.ts` のセッション三値（`unknown` / `active` /
`inactive`）のうち **`inactive` のときだけ許可**する。Service Worker は
...
キューの各エントリには**積んだ時点のヒーローのUserId**も持たせる。取得は
ページ側の認証エンベロープで行われるので、繰り延べ中に別アカウントへログイン
すると旧アカウントのHandIdが新しい資格情報で `2302` になり、再試行不能として
永久に捨てられてしまう。積んだアカウントと一致するときだけ撃つ。
