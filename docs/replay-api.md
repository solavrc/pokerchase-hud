...
判定は2段。`update-manager.ts` のセッション三値（`unknown` / `active` /
`inactive`）が **`inactive`** で、かつ**接続中の全ポート（＝ゲームタブ）が
`inactive`** であることを要求する。前者は全タブを畳んだ1つの値なので、
タブAで対局中でもタブBの309で `inactive` へ倒れてしまう ―― 畳んだ値は
forced update と共有していて意味を変えられないので、**取得側の判定点だけ**を
厳しくしている（`replay-port-state.ts`）。状態の分からないポートは対局中扱い、
接続が1つも無ければ取得しない（そもそもページ経由でしか撃てない）。
対局中のまま切れたタブは、再接続の猶予（60秒）の間も対局中として扱う ――
`RuntimePortManager` は切断の500ms後に再接続するので、その隙にポートが
`connectedPorts` から消えると、残った非対局タブだけで条件が成立してしまう。Service Worker は
いつでも落ちうるので、再起動直後の `unknown` は「セッション中かもしれない」が
正しい状態であり、そこで撃つと不変条件を破りうる。分からない間は積んだまま
待ち、次のセッション終了で取得する。
...
キューの各エントリには**そのハンドを送ってきたタブのヒーローのUserId**も
持たせ、依頼は
**そのアカウントを観測したタブへだけ**出す。取得はページ側の認証エンベロープで
行われるので、別アカウントのタブへ投げると `2302` が返り、再試行不能として
永久に捨てられてしまう。一致するタブが無ければ撃たずにキューへ残し、正しい
アカウントで繋ぎ直したときに流れる。
