...
2026-08-01 の raw capture（561,309行）には配札（303）が41,577件あった。
保存順に303から306までを走査すると、306で閉じた配札は41,008件、306より先に
次の303が来た先取り（hand preemption）は568件、終端で未クローズは1件だった。
先取りされた303と次の303の記録時刻差は最短13,619ms。この集計は保存ログの
記述的な観測であり、タブ間の実到着順や同時配信の有無を直接測定したものではない。
このゲーム側制約を前提に、WebSocket由来のgame eventを最後に
