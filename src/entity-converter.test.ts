...
describe('EntityConverter', () => {
  ...
  describe('phase membership, winner definition, and river-call-won parity', () => {
    ...
    /**
     * DEAL_ROUND省略プリフロップ全員オールイン + サイレントタイムアウト混在
     * (PR #184 codex review, P2)。
     *
     * scenario dの3人卓に、EVT_ACTIONを一切送らず（明示的なFOLDなし）かつ
     * EVT_HAND_RESULTS.Resultsにも一切現れない4人目 D（タイムアウト/切断を
     * 模したシグネチャ、docs/api-events.md「EVT_ACTION: 送信されないケース」
     * 「タイムアウト / 切断」）を加える。C(BTN)は通常通り明示的にFOLDし、
     * A(SB)がオールイン、B(BB)がコール（オールイン一致）してEVT_DEAL_ROUNDが
     * 一度も来ないままショーダウンへ進む。
     *
     * 修正前はFLOP合成メンバーシップを「PREFLOPでFOLDアクションを送らなかった
     * 全員」で判定していたため、Dは明示的なFOLDを送っていない以上フロップを
     * 見たことにされてしまい（WTSD/WWSFの分母を水増しする）、EVT_HAND_RESULTS.
     * ResultsにDが存在しないという事実が無視されていた。
     * 修正後は「PREFLOP FOLDなし AND Results[]に存在する」のANDで判定するため、
     * D は除外され、Results[]に実在するA・Bのみが合成FLOPメンバーになる。
     */
    it('excludes a silent-timeout player (no FOLD action, absent from Results) from the synthesized FLOP in BOTH pipelines', async () => {
      const A = 700411, B = 700422, C = 700433, D = 700444
      const events: ApiEvent[] = [
        createEvent(ApiType.EVT_DEAL, {
          timestamp: 71000,
          SeatUserIds: [A, B, C, D],
        ...
        // D(seat3)はサイレントタイムアウト：EVT_ACTIONを一切送らない
        // （Progress.NextActionSeat=3で期待されていたが、次に届くのはC(seat2)の
        // FOLD。この不一致自体はハンド破棄の根拠にならない正常系
        // — docs/api-events.md「NextActionSeat不一致はハンド破棄の根拠にならない」）。
        // C(BTN, seat2)が明示的にフォールド
        createEvent(ApiType.EVT_ACTION, {
          timestamp: 71001,
          SeatIndex: 2,
          ActionType: ActionType.FOLD,
        ...
        createEvent(ApiType.EVT_HAND_RESULTS, {
          ...
          ResultType: 0,
          DefeatStatus: 0,
          // C(明示的フォールド、FOLD_OPENなし)とD(タイムアウト)は
          // Results[]に一切現れない — 両者とも「フォールドしなかった
          // プレイヤーのみ」ルールに合致しない
          // (src/types/api.ts EVT_HAND_RESULTS.Results describe)。
          Results: [
            {
              UserId: A,
              HoleCards: [37, 51],
      ...
      expect([...importFlopPhase!.seatUserIds].sort((a, b) => a - b)).toEqual([A, B])
      expect(importFlopPhase!.seatUserIds).not.toContain(C)
      expect(importFlopPhase!.seatUserIds).not.toContain(D)
      expect(importFlopPhase!.communityCards).toEqual([1, 21, 44])
      ...
      const livePhases = await pipeEventsAndWaitForReady(
        ...
        async () => {
          await dbMock.open()
          return dbMock.phases.where('handId').equals(700423456).toArray()
        },
        result => result.length >= importResult.phases.length,
        { description: `dbMock.phases with handId=700423456 to have >= ${importResult.phases.length} row(s)` }
      )
      ...
      expect([...liveFlopPhase!.seatUserIds].sort((a, b) => a - b)).toEqual([A, B])
      expect(liveFlopPhase!.seatUserIds).not.toContain(C)
      expect(liveFlopPhase!.seatUserIds).not.toContain(D)
      expect(liveFlopPhase!.communityCards).toEqual([1, 21, 44])
