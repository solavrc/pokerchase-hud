...
const baseSchema = z.object({
  ...
  sequence: z.number().int().nonnegative().optional().describe('同一timestamp+ApiTypeId内の0始まり保存順序')
...
export const apiEventSchemas = {
  ...
  [ApiType.EVT_DEAL]: baseSchema.extend({
    ...
    Progress: z.object({
      ...
      Pot: z.int().nonnegative().describe('配札時点のポット額。**アンテだけでなく投稿済みブラインド（SB+BB）も含む**（poker-warehouse I1監査で全数検証: 29,715/29,715本番ハンド+2キャプチャでΣ(Chip)+Pot+ΣSidePot==table_size×DefaultChipが成立、BetChipを別途加算すると二重計上になる）。ただしアンテまたはブラインドの強制投稿で配札時点のオールインキャップが発生している場合はこの限りでない: Pot/SidePotは既にオールイン上限を反映したティア別スナップショットになる。アンテのみオールインではブラインドは最上位（未キャップ）のサイドポットにのみ含まれ、Pot自体はアンテ拠出者数で割り切れるブラインド抜きの額になる。ショートスタック推定（下記）はこの分割後の値を前提に組まれている'),
