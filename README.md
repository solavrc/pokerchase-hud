...
ポップアップは実際の拡張機能と同じく右上に実寸で表示され、そこで変更した表示設定（サイズ・表示/非表示・コンパクト/フル・統計カラー・表示する統計）は本番と同様その場でHUDに反映されます。一方、データを伴う操作（エクスポート・インポート・データ再構築）は、背景で処理を行うサービスワーカーもデータベースも存在しないため、いずれも失敗としてエラー表示されます。
