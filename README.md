...
- **プレイヤー別の詳細表示**: ポジション別統計と、最近のハンドをHUDから確認できます。直近ハンドは表示件数を10・25・50・100件から選べ、「参加のみ」（既定ON）でブラインドを取られただけのハンドを隠せます（どちらの選択も端末ごとに保存されます）。そのプレイヤーが実際に配られたハンドだけを、ホールカード（4色表示。自分のハンドはショーダウンに関わらず常に表示、相手はショーダウンで公開されたもの）・プリフロップのライン・フロップ以降のアクション・BB単位の損益とあわせて一覧します。
